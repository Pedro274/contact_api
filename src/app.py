...
class Agenda_List(Resource):
    ...
    def post(self):
        ...
        agenda_name = next(filter(lambda agenda: agenda["agenda_name"] == data["agenda_name"], agendas), None)
        if agenda_name:
...
class Agenda(Resource):
    def get(self, name):
        contact = next(filter(lambda contact: contact["agenda_name"] == name, agendas), None)  
        if contact:
            return contact
        else:
    ...
    def delete(self, name):
        ...
        for agenda in agendas:
            if agenda["agenda_name"] == name:
                agendas = list(filter(lambda agenda: agenda["agenda_name"] != name, agendas))
                return agendas
        return {"message": "Sorry this contact does not exist"}
...
class Contact(Resource):
    ...
    def get(self, name):
        agenda = next(filter(lambda contact: contact["agenda_name"] == name, agendas), None)
        if agenda:
    ...
    def post(self, name):
        ...
        agenda = next(filter(lambda agenda: agenda["agenda_name"] == name, agendas), None)
        if agenda:
            contact = next(filter(lambda contact: contact["name"] == data["name"], agenda["agenda_contact"]), None)
            if contact:
    ...
    def put(self, name):
        ...
        agenda = next(filter(lambda agenda: agenda["agenda_name"] == name, agendas), None)
        if agenda:
            contact = next(filter(lambda contact: contact["id"] == data["id"], agenda["agenda_contact"]), None)
            if contact:
                contact.update(data)
                return contact
            else:
